...
x, y, z, roll, yaw, pitch, flex = 0

try:
  while True:
    # Non-blocking keyboard input setup
    if 'old_settings' not in globals():
    ...
    step_pos = 0.005   # position step (meters)
    step_rot = 2.0     # rotation step (degrees)
    step_flex = 0.05   # gripper step

    if key:
        if key == 'w': z += step_pos
        elif key == 's': z -= step_pos
        elif key == 'a': y -= step_pos
        elif key == 'd': y += step_pos
        elif key == 'q': x -= step_pos
        elif key == 'e': x += step_pos

        elif key == 'i': pitch += step_rot
        elif key == 'k': pitch -= step_rot
        elif key == 'j': yaw -= step_rot
        elif key == 'l': yaw += step_rot
        elif key == 'u': roll -= step_rot
        elif key == 'o': roll += step_rot

        elif key == 'n': flex -= step_flex
        elif key == 'm': flex += step_flex
